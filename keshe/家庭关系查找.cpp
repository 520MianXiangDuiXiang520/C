...
person *findname(person *t, char findname[20] )//根据姓名查找
{
	...
	while ((k != NULL) || (s->top != 0))
	{
		while (k)
		{
			if (strcmp(k->record.name, findname) == 0)
			{
				return k;
			}
			push(s, k);
...
void children(person *q,char dai[10])//寻找后三代任意一代的信息
{
	...
	else
	{
		...
		q = q->child;
		if (strcmp(dai, "1")==0)
			outtextxy(400, Buttom + 50, q->record.name);
			/*display(q);*/
	...
	while (!qq.empty() && (qq.front() != p))
	{
		...
		if (q->child)
		{
			...
			if (strcmp(dai, "2")==0)
			{
				outtextxy(400, Buttom + jianju2, q->record.name);
				jianju2 += 30;
				kk++;
			}
			while (q->brother)
	...
	while ((t) || (s->top > 0))
	{
		...
		while ((s->top > 0) && (s->tag[s->top - 1] == 1))
		{
			...
			if (strcmp(t->record.name, q->record.name) == 0)
			{
				...
				jianju1 = 50;
				if ((s->tag[s->top - 1] == 1) && (s->top > 0) && (k == 0))
				{
					/*printf("%s的兄弟为\n", q->record.name);*/
					outtextxy(300, Buttom + 20, "他的兄弟为：");
				}
